package r0;

import android.os.Build;
import java.util.List;
import kotlin.Unit;
import kotlin.jvm.functions.Function2;
import kotlin.jvm.internal.Intrinsics;

/* loaded from: classes.dex */
public final class c extends iu.k implements Function2 {

    /* renamed from: a, reason: collision with root package name */
    public final /* synthetic */ int f32340a;

    /* renamed from: b, reason: collision with root package name */
    public final /* synthetic */ Object f32341b;

    /* renamed from: c, reason: collision with root package name */
    public final /* synthetic */ Object f32342c;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public /* synthetic */ c(int i10, Object obj, Object obj2) {
        super(2);
        this.f32340a = i10;
        this.f32341b = obj;
        this.f32342c = obj2;
    }

    public final void a(t0.n nVar, int i10) {
        g1.o oVar = g1.l.f16404b;
        int i11 = this.f32340a;
        Object obj = this.f32342c;
        Object obj2 = this.f32341b;
        int i12 = 3;
        switch (i11) {
            case 1:
                if ((i10 & 3) == 2) {
                    t0.r rVar = (t0.r) nVar;
                    if (rVar.G()) {
                        rVar.V();
                        return;
                    }
                }
                g1.o s7 = androidx.compose.foundation.layout.a.s(androidx.compose.foundation.layout.d.a(oVar, z.f32822b, z.f32823c), (b0.h1) obj2);
                b0.g gVar = b0.n.f2939e;
                g1.f fVar = g1.a.f16389k;
                hu.c cVar = (hu.c) obj;
                t0.r rVar2 = (t0.r) nVar;
                rVar2.b0(693286680);
                b0.l1 a10 = b0.o1.a(gVar, fVar, rVar2, 54);
                rVar2.b0(-1323940314);
                int i13 = rVar2.P;
                t0.r1 o10 = rVar2.o();
                c2.l.M.getClass();
                c2.j jVar = c2.k.f7746b;
                b1.c j10 = androidx.compose.ui.layout.a.j(s7);
                if (rVar2.f35166a instanceof t0.f) {
                    rVar2.e0();
                    if (rVar2.O) {
                        rVar2.n(jVar);
                    } else {
                        rVar2.n0();
                    }
                    t0.t.v0(rVar2, a10, c2.k.f7749e);
                    t0.t.v0(rVar2, o10, c2.k.f7748d);
                    c2.i iVar = c2.k.f7750f;
                    if (rVar2.O || !Intrinsics.a(rVar2.Q(), Integer.valueOf(i13))) {
                        nn.d.s(i13, rVar2, i13, iVar);
                    }
                    v.e.w(rVar2, j10, rVar2, 0, 2058660585);
                    cVar.b(b0.q1.f2981a, rVar2, 6);
                    rVar2.s(false);
                    v.e.x(rVar2, true, false, false);
                    return;
                }
                al.d.v0();
                throw null;
            case 2:
                if ((i10 & 3) == 2) {
                    t0.r rVar3 = (t0.r) nVar;
                    if (rVar3.G()) {
                        rVar3.V();
                        return;
                    }
                }
                g1.o a11 = h2.l.a(oVar, false, d0.f32360c);
                b2.i iVar2 = b0.a2.f2821a;
                g1.o H = t0.t.H(a11, d2.s.f13613k, new v.g((b0.x1) obj2, i12));
                if (Build.VERSION.SDK_INT >= 33) {
                    oVar = androidx.compose.foundation.layout.a.n(oVar);
                }
                g1.o g10 = H.g(oVar);
                t0.l3 l3Var = (t0.l3) obj;
                t0.r rVar4 = (t0.r) nVar;
                rVar4.b0(733328855);
                b0.v c10 = b0.s.c(g1.a.f16379a, false, rVar4, 0);
                rVar4.b0(-1323940314);
                int i14 = rVar4.P;
                t0.r1 o11 = rVar4.o();
                c2.l.M.getClass();
                c2.j jVar2 = c2.k.f7746b;
                b1.c j11 = androidx.compose.ui.layout.a.j(g10);
                if (rVar4.f35166a instanceof t0.f) {
                    rVar4.e0();
                    if (rVar4.O) {
                        rVar4.n(jVar2);
                    } else {
                        rVar4.n0();
                    }
                    t0.t.v0(rVar4, c10, c2.k.f7749e);
                    t0.t.v0(rVar4, o11, c2.k.f7748d);
                    c2.i iVar3 = c2.k.f7750f;
                    if (rVar4.O || !Intrinsics.a(rVar4.Q(), Integer.valueOf(i14))) {
                        nn.d.s(i14, rVar4, i14, iVar3);
                    }
                    v.e.w(rVar4, j11, rVar4, 0, 2058660585);
                    ((Function2) l3Var.getValue()).invoke(rVar4, 0);
                    rVar4.s(false);
                    rVar4.s(true);
                    rVar4.s(false);
                    rVar4.s(false);
                    return;
                }
                al.d.v0();
                throw null;
            default:
                if ((i10 & 3) == 2) {
                    t0.r rVar5 = (t0.r) nVar;
                    if (rVar5.G()) {
                        rVar5.V();
                        return;
                    }
                }
                ((hu.c) obj2).b((List) obj, nVar, 0);
                return;
        }
    }

    @Override // kotlin.jvm.functions.Function2
    public final Object invoke(Object obj, Object obj2) {
        switch (this.f32340a) {
            case 0:
                float floatValue = ((Number) obj).floatValue();
                float floatValue2 = ((Number) obj2).floatValue();
                t tVar = ((p) this.f32341b).f32664a;
                tVar.f32724i.j(floatValue);
                tVar.f32725j.j(floatValue2);
                ((iu.w) this.f32342c).f20557a = floatValue;
                return Unit.f23355a;
            case 1:
                a((t0.n) obj, ((Number) obj2).intValue());
                return Unit.f23355a;
            case 2:
                a((t0.n) obj, ((Number) obj2).intValue());
                return Unit.f23355a;
            default:
                a((t0.n) obj, ((Number) obj2).intValue());
                return Unit.f23355a;
        }
    }
}
